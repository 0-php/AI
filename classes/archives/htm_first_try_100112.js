...
htm = {
	...
	activeColumns: [],  //List of column indices that are winners due to bottom-up input
	desiredLocalActivity: 10, //A parameter controlling the number of columns that will be winners after the inhibition step
	inhibitionRadius: 100, //Average connected receptive field size of the columns
	...
	activeDutyCycle: 0, //A sliding average representing how often column c has been active after inhibition (e.g. over the last 1000 iterations)
	overlapDutyCycle: 0, //A sliding average representing how often column c has had significant overlap (i.e. greater than minOverlap) with its inputs (e.g. over the last 1000 iterations)
	...
	activeColumns: [], //List of column at defined time indices that are winners due to bottom-up input (this is the output of the spatial pooler)
	...
	spatialPooler: function(){
		...
		for(c in this.columns){
			...
			for(s in this.connectedSynapses(c))
				column.overlap = column.overlap + input(t, s.sourceInput);
		...
		for(c in this.columns){
			...
			if(column.overlap > 0 && column.overlap >= this.minLocalActivity)
				this.activeColumns(t).append(c);
		}
		//Phase 3: Learning
		for(c in this.activeColumns(t)){
			column = this.activeColumns(t);
		...
		for(c in this.columns){
			column = this.columns[c];
			this.minDutyCycle[c] = 0.01 * this.maxDutyCycle(this.neighbors(c));
	...
	temporal_pooler: function(){
		...
		for(c, i in cells){
			//Cell predicted it's future state
			if(this.earnState(s, i, t) == 1)
