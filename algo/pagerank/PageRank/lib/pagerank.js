...
//Use a random surfer algorithm to determine the relative rank of nodes. The importance of each node is determined
//by the number of incoming links as well as the importance of those incoming links.
...
Pagerank.prototype.iterate = function(count){
	...
	//For each node, we look at the incoming edges and the weight of the node connected via each edge.
	//This weight is divided by the total number of outgoing edges from each weighted node and summed to determine the new weight of the original node.
