...
# of Bi-gram Files	Processors    Memory required per Processor  	Wall Clock Time(using MPI_Wtime)
	...
	32		   4			5.8 GB				864.928039 Seconds
	...
	{	
		...
		else
		{
			...
			{	
				...
				{
					index2 = binarySearch(ch1,uSize,word);
				...
				{
					index1 = binarySearch(ch,uSize,word);
					index2 = binarySearch(ch1,uSize,word);
...
node** read_unigram_data(char* path, int n,node** word)
{
	...
	if((fpuni=fopen(files,"r"))==NULL)
        ...
        {
		word = initialize_word(word, uSize);
			...
			fscanf(fpuni,"%ld",&j);
                	...
                	word[count]->token=(char *)malloc(sizeof(char)*(strlen(ch)+1));
