...
if($_POST) { //form action
	if($app->model->val->run("network.data",$_POST)) {
		...
		$id = $app->model->get("network")->add($_POST['label'],$authkey,"managed",$_POST['lr'],$momentum,$_POST['mse'],$_POST['epoch']);
		
		//Create the new network instance
		$nn = new nmesh($_POST['inputs'],$_POST['outputs'],$_POST['neurons'],$_POST['layers'],
						$_POST['bias'],$_POST['wrange']);
		
		$app->model->get("network")->save($nn,$id); //insert the snapshot
		$app->model->get("usernetwork")->link($_SESSION['id'],$id);
