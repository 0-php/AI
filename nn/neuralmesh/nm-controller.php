...
class NeuralMesh {
	...
	static public function createNetwork($input,$output,$hidden,$layers) {
		...
		$authkey = sha1(uniqid());
		$id = self::$app->model->get("network")->add("Temp",$authkey,"unmanaged");
		self::$app->model->get("network")->save($nn,$id);
...
class UnmanagedNetwork extends AbstractNetwork {
	...
	public function train($inputs,$outputs,$epochs=30) {
		...
		NeuralMesh::$app->model->get("network")->save($this->nn,$id);
	}
	...
	public function destory() {
		NeuralMesh::$app->model->get("network")->destory($this->id);
	}
}
...
class ManagedNetwork extends AbstractNetwork {
	...
	public function train($inputs,$outputs,$epochs=30) {
		...
		$data = $this->nn->quick_train($id,$epochs,$lr,$inputs,$outputs);
		NeuralMesh::$app->model->get("epochs")->saveEpoch($id,$epochs,$data['startmse'],$data['endmse'],$data['time']);
		NeuralMesh::$app->model->get("cache")->quickCache($id,$inputs,nmesh::$cache);
	...
	public function run($inputs) {
		$id = $this->id;
		$data = NeuralMesh::$app->model->get("cache")->getCache($id.$inputs);
		
		if($data === null) { //not found in cache
			$inputarray = explode("|",$inputs);
			if(count($inputarray) != $this->nn->inputs) 
				die("Incorrect number of entries! Expected ".$this->nn->inputs." got ".count($inputarray));
			$outputs = $this->nn->run($inputarray);
			//save into cache
			$hash = $id.$inputs;
			NeuralMesh::$app->model->get("cache")->saveCache($hash,$id,implode("|",$outputs));
