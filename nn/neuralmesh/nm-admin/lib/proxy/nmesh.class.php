...
class nmesh
{
	...
	function run($inputarray){
		...
		for($l = 0; $l < $l_count; ++$l){ // Now we walk through each layer of the net
			foreach($this->layer[$l]->neuron as $neuron){
				...
				$value = -1 + 2 / (1 + exp(-1 * ($sum + $neuron->bias))); //bipolar sigmoid
				$neuron->value = $value;
				$output[$l + 1][] = $value;
			}
		}
	...
	function calculate_deltas($outputarray, $lr)
	{
		...
		for($l = $l_count; $l >= 0; --$l){
			...
			//TODO: use temp var then add to array rather than init with this thing
			...
			foreach($this->layer[$l]->neuron as $n=>$neuron){
				...
				$delta = $n_error *  0.5 * (1 + $neuron->value) * (1 - $neuron->value); //bipolar sigmoid derivative
	...
	function remove_outputs($outputs){
		if($this->outputs - $outputs < 1)
			throw new Exception("Cannot remove that many outputs!");
	...
	function remove_layer($layer = null){
		...
		if(count($this->layer) > 2 && $layer < count($this->layer)-1){
